...
public class ReceiptDoubleEntryActivity extends AppCompatActivity implements View.OnClickListener, AccountMasterSelectedListener, EntryEditedListener, EntryDeletedListener {
    private ReceiptDoubleEntryActivity activity = ReceiptDoubleEntryActivity.this;
    private TextView tVReceiptDate, tVReceiptNumber, tVAddReceiptAccounts, tVCancelReceiptDoubleEntry, tVSaveReceiptDoubleEntry, tVAccountName,
            tVClosingBalance, tVNarration, tVDebitAmount, tVCreditAmount, tVDifference;
    private CardView cVNarration, cVAddReceiptAccounts;
    ...
    private JSONArray receiptArraySubmit;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_receipt_double_entry);
        init();
        getVoucherNumber();
        setDate();
    }
    ...
    private void init() {
        ...
        tVCancelReceiptDoubleEntry = findViewById(R.id.tVCancelReceiptDoubleEntry);
        tVSaveReceiptDoubleEntry = findViewById(R.id.tVSaveReceiptDoubleEntry);
        ...
        receiptArraySubmit = new JSONArray();
    ...
    private void initListeners() {
        ...
        tVCancelReceiptDoubleEntry.setOnClickListener(activity);
        tVSaveReceiptDoubleEntry.setOnClickListener(activity);
    ...
    private void getVoucherNumber() {
        progressDialog.show();
        String url = APIs.receiptVoucherNumber + preferences.getString(AppTexts.orgId, "") + "/" + preferences.getString(AppTexts.fyId, "");
...
//            progressDialog.dismiss();
            try {
                ...
                if (response.getString(AppTexts.status).equalsIgnoreCase(AppTexts.success)) {
                    ...
                    tVReceiptNumber.setText(voucherNumber);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.tVCancelReceiptDoubleEntry:
                onBackPressed();
                finish();
                break;
            ...
            case R.id.tVSaveReceiptDoubleEntry:
    ...
    private void makeJsonSubmit() {
        ...
        for (int i = 0; i < listSize; i++) {
            ...
            receiptArraySubmit.put(body);
        }
    }

    private void sendReceiptEntry() {
        progressDialog.show();
        String url = APIs.saveReceiptEntry + preferences.getString(AppTexts.orgId, "") + "/" + preferences.getInt(AppTexts.userId, 0) + "/" + preferences.getString(AppTexts.fyId, "");
        ...
        try {
            ...
            obj.put("transactionDetailDTOList", receiptArraySubmit);
        } catch (JSONException e) {
            e.printStackTrace();
        }
...
//        AppUtil.customizeRequest(itemServiceResponse);
    ...
    private void populateAccountMaster(List<AccountMasterDTO> accountMasterDTOList) {
        ...
        accountMasterAdapter = new AccountMasterAdapter(activity, accountMasterDTOList, activity, "receiptEntry");
