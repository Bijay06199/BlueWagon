...
import com.example.bluewagon.ecalculo.adapters.reportsAdapter.CashBookAdapter;
...
import com.example.bluewagon.ecalculo.dtos.CashBookDTO;
...
public class CashBookReportActivity extends AppCompatActivity implements View.OnClickListener, AccountMasterSelectedListener {
    private CashBookReportActivity activity = CashBookReportActivity.this;
    ...
    private TextView tVDateFrom, tVDateTo, tVTitle, tVCancelCashBook, tVAccountName;

    private LinearLayout lLDateFrom, lLDateTo, lLNoData;

    private HorizontalScrollView scrollMain;
    private RecyclerView rVCashBook;
    ...
    private void init() {
        ...
        tVCancelCashBook = findViewById(R.id.tVCancelCashBook);
        ...
        tVTitle.setText("Cash Book");

        rVCashBook.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void initListeners() {
        ...
        tVCancelCashBook.setOnClickListener(activity);
    ...
    private void initRecyclerView() {
        cashBookDTOList = new ArrayList<>();
        cashBookAdapter = new CashBookAdapter(activity, cashBookDTOList);

        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rVCashBook.setLayoutManager(manager);
        rVCashBook.setAdapter(cashBookAdapter);
    }
...
//            dialog.setMinDate(sModel);
    ...
    private void getValues() {
        ...
        if (fromDate.equals(rDate) || toDate.equals(rDate)) {
            ...
            initRecyclerView();
            getCashBook(dateFrom, dateTo);
        }
    }

    private void getCashBook(String startDate , String endDate){
        progressDialog.show();
        //1 is for account master Id
        String url = APIs.cashBook + preferences.getString(AppTexts.orgId, "") + "/" + accountId + "/" + preferences.getString(AppTexts.fyId, "") + "/" + startDate + "/" + endDate + "/";
            ...
            try {
                if (response.getString(AppTexts.status).equals(AppTexts.success)) {
                    ...
                    for (int i = 0; i < listArray.length(); i++) {
                        CashBookDTO dto = new Gson().fromJson(listArray.getJSONObject(i).toString(), CashBookDTO.class);
                        cashBookDTOList.add(dto);
                    }
                    if (cashBookDTOList.size() > 0) {
                        lLNoData.setVisibility(View.GONE);
                        rVCashBook.setVisibility(View.VISIBLE);
                        ...
                        rVCashBook.setVisibility(View.GONE);
                    }
                } else {
                    progressDialog.dismiss();
