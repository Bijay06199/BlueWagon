...
import com.example.bluewagon.ecalculo.adapters.transactionAdapter.view.SalesOrderAdapter;
...
public class SalesOrderViewActivity extends AppCompatActivity implements View.OnClickListener{
    private SalesOrderViewActivity activity = SalesOrderViewActivity.this;
    ...
    private void init() {
        ...
        rVSalesOrder = findViewById(R.id.rVSalesOrder);
        ...
        tVTitle.setText("Sales Order View");

        rVSalesOrder.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void initRecyclerView() {
        salesOrderDTOList = new ArrayList<>();
        salesOrderAdapter = new SalesOrderAdapter(activity, salesOrderDTOList);

        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rVSalesOrder.setLayoutManager(manager);
        rVSalesOrder.setAdapter(salesOrderAdapter);
    }
...
//            dialog.setMinDate(sModel);
    ...
    private void getInvoiceView(String startDate, String endDate){
        progressDialog.show();
        String url = APIs.salesOrder + preferences.getString(AppTexts.orgId, "") + "/" + preferences.getInt(AppTexts.userId, 0) + "/"  + startDate + "/" + endDate ;
            ...
            try {
                if (response.getString(AppTexts.status).equals(AppTexts.success)) {
                    ...
                    for (int i = 0; i < listArray.length(); i++) {
                        SalesOrderDTO dto = new Gson().fromJson(listArray.getJSONObject(i).toString(), SalesOrderDTO.class);
                        salesOrderDTOList.add(dto);
                    }
                    if (salesOrderDTOList.size() > 0) {
                        lLNoData.setVisibility(View.GONE);
                        rVSalesOrder.setVisibility(View.VISIBLE);
                        ...
                        rVSalesOrder.setVisibility(View.GONE);
                    }
                } else {
                    progressDialog.dismiss();
