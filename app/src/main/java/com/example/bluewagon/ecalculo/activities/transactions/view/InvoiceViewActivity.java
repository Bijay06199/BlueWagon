...
import com.example.bluewagon.ecalculo.adapters.transactionAdapter.view.SalesInvoiceViewAdapter;
import com.example.bluewagon.ecalculo.dtos.SalesPurchaseReportDTO;
...
public class InvoiceViewActivity extends AppCompatActivity implements View.OnClickListener {
    private InvoiceViewActivity activity = InvoiceViewActivity.this;
    ...
    private RecyclerView rVSalesInvoice;
    private SharedPreferences preferences;
    private androidx.appcompat.app.AlertDialog progressDialog;

    private List<SalesPurchaseReportDTO> salesPurchaseReportDTOList;
    private SalesInvoiceViewAdapter salesInvoiceViewAdapter;
    ...
    private void init() {
        ...
        rVSalesInvoice = findViewById(R.id.rVSalesInvoice);
        ...
        String title = getIntent().getStringExtra(AppTexts.salesInvoice);
        tVTitle.setText(title);

        rVSalesInvoice.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void initRecyclerView() {
        salesPurchaseReportDTOList = new ArrayList<>();
        salesInvoiceViewAdapter = new SalesInvoiceViewAdapter(activity, salesPurchaseReportDTOList);

        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rVSalesInvoice.setLayoutManager(manager);
        rVSalesInvoice.setAdapter(salesInvoiceViewAdapter);
    }
...
//            dialog.setMinDate(sModel);
    ...
    private void getInvoiceView(String startDate, String endDate) {
        progressDialog.show();
        String url = APIs.salesInvoice + preferences.getString(AppTexts.orgId, "") + "/" + preferences.getInt(AppTexts.userId, 0) + "/" + startDate + "/" + endDate;
            ...
            try {
                if (response.getString(AppTexts.status).equals(AppTexts.success)) {
                    ...
                    for (int i = 0; i < listArray.length(); i++) {
                        SalesPurchaseReportDTO dto = new Gson().fromJson(listArray.getJSONObject(i).toString(), SalesPurchaseReportDTO.class);
                        salesPurchaseReportDTOList.add(dto);
                    }
                    if (salesPurchaseReportDTOList.size() > 0) {
                        lLNoData.setVisibility(View.GONE);
                        rVSalesInvoice.setVisibility(View.VISIBLE);
                        scrollMain.setVisibility(View.VISIBLE);
                        salesInvoiceViewAdapter.notifyDataSetChanged();
                        progressDialog.dismiss();
                    } else {
                        progressDialog.dismiss();
                        lLNoData.setVisibility(View.VISIBLE);
                        scrollMain.setVisibility(View.GONE);
                        rVSalesInvoice.setVisibility(View.GONE);
                    }
                } else {
                    progressDialog.dismiss();
