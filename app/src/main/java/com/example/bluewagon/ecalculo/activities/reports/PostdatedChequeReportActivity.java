...
import com.example.bluewagon.ecalculo.adapters.reportsAdapter.PostDatedChequeAdapter;
...
public class PostdatedChequeReportActivity extends AppCompatActivity implements View.OnClickListener, AccountMasterSelectedListener {
    private PostdatedChequeReportActivity activity = PostdatedChequeReportActivity.this;
    ...
    private RecyclerView rVPostDatedCheque;
    private PostDatedChequeAdapter postDatedChequeAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_postdated_cheque_report);
        init();
        initRecyclerView();
        checkAndSetDates();
        getBankList();
    ...
    private void init() {
        ...
        rVPostDatedCheque = findViewById(R.id.rVPostDatedCheque);
        ...
        tVTitle.setText("Post Dated Cheque Report");

        rVPostDatedCheque.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
    ...
    private void initRecyclerView() {
        ...
        postDatedChequeAdapter = new PostDatedChequeAdapter(activity, accountStatementLedgerDTOSList);

        FixedGridLayoutManager manager = new FixedGridLayoutManager();
        manager.setTotalColumnCount(1);
        rVPostDatedCheque.setLayoutManager(manager);
        rVPostDatedCheque.setAdapter(postDatedChequeAdapter);
    }
...
//        getLedger(fromDate, toDate);
    ...
    private void getReportType() {
        ...
        spinnerArray.add("clear");
        spinnerArray.add("unclear");
...
//            dialog.setMinDate(sModel);
    ...
    private void getBankLedger(String startDate, String endDate) {
        ...
        String url = APIs.bankPdcTransaction + preferences.getString(AppTexts.orgId, "") + "/" + preferences.getString(AppTexts.fyId, "") + "/" + bankId + "/" + startDate + "/" + endDate + "/" + spinnerReportType.getSelectedItem().toString();
            ...
            try {
                if (response.getString(AppTexts.status).equals(AppTexts.success)) {
                    ...
                    if (accountStatementLedgerDTOSList.size() > 0) {
                        ...
                        rVPostDatedCheque.setVisibility(View.VISIBLE);
                        scrollMain.setVisibility(View.VISIBLE);
                        postDatedChequeAdapter.notifyDataSetChanged();
                        progressDialog.dismiss();
                    } else {
                        progressDialog.dismiss();
                        lLNoData.setVisibility(View.VISIBLE);
                        scrollMain.setVisibility(View.GONE);
                        rVPostDatedCheque.setVisibility(View.GONE);
                    }
                } else {
                    progressDialog.dismiss();
